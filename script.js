...
function isCharAVowel(char) {
    // loop through vowels and compare against given value
    for (i = 0; i < vowels.length; i++) {
        if (char == vowels[i]) {
            return true;
        }
    }
    return false; // return false if loop never returned true
}
...
function maxOfThree(num1, num2, num3) {
    ...
    // check if 3rd value is a bigger number
    if (num3 > largerNum) {
        largerNum = num3;
    }
...
function convertTemperature(temp, scale) {
    if (scale.toUpperCase() === 'C') // toUpperCase() to make it case insensitive
    {
        ...
        return "(Error: incorrect scale type.)";
    }
}
